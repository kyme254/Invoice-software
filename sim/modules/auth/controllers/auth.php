...
class Auth extends MX_Controller {
	...
	function logout()
	{
		...
		$this->session->set_flashdata('success_message', $this->lang->line('logged_out'));
	...
	function change_password()
	{
		...
		else
		{
			...
			{ //if the password was successfully changed
				...
				$logout = $this->ion_auth->logout();
				$this->session->set_flashdata('success_message', $this->lang->line('password_changed'));
				redirect('module=auth&view=login', 'refresh');
			}
			else
			{
	...
	function delete_user($id = NULL)
	{
		...
		{ //check to see if we are deleting the biller
			//redirect them back to the admin page
			$this->session->set_flashdata('message', $this->lang->line("user_deleted"));
			redirect("module=auth&view=users", 'refresh');
		}
